...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_RELAY, DP_TYPE_BOOL},
  {DPID_NEC_IT, DP_TYPE_STRING},
  {DPID_NEC_IR, DP_TYPE_STRING},
  {DPID_LIGHT_LESEN, DP_TYPE_VALUE},
  {DPID_INFRARED, DP_TYPE_ENUM},
  {DPID_LEDRED, DP_TYPE_BOOL},
  {DPID_LEDBLUE, DP_TYPE_BOOL},
	{DPID_WIFI_CONTROL, DP_TYPE_BOOL},
};
...
void all_data_update(void)
{
  ...
  //  #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    ...
    mcu_dp_value_update(DPID_HUMIDITY_VALUE,50); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MAXTEMP_SET,temp_max); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MINITEMP_SET,temp_min); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MAXHUM_SET,rh_max); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MINIHUM_SET,rh_min); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_RELAY,1); //BOOL型数据上报;
    mcu_dp_string_update(DPID_NEC_IR,Mec_Code,6); //STRING型数据上报;
    mcu_dp_value_update(DPID_LIGHT_LESEN,50); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_INFRARED,0); //枚举型数据上报;
    mcu_dp_bool_update(DPID_LEDRED,!LED_IN_R); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_LEDBLUE,!LED_IN_G); //BOOL型数据上报;
		mcu_dp_bool_update(DPID_WIFI_CONTROL,0); //BOOL型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_relay_handle
功能描述 : 针对DPID_RELAY的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_relay_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    relay = mcu_get_dp_download_bool(value,length);
    RELAY=relay;
  
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_RELAY,relay);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_nec_it_handle
功能描述 : 针对DPID_NEC_IT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 只下发类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_nec_it_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为STRING
    unsigned char ret;
    /*
    ...
    */
	...
	if(value[3]==0x21)
		 {
				NEC_IE_code_message(Mec_Code[0]+Mec_Code[1]*256,Mec_Code[2]);//标准协议
		 }
	else
		{
		
			NEC_IE_code_message_Meidi(Mec_Code[0],Mec_Code[1],Mec_Code[2]);//美的协议
		
		}
	
	
    //处理完DP数据后应有反馈
    ret = mcu_dp_string_update(DPID_NEC_IT,value, length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_nec_ir_handle
功能描述 : 针对DPID_NEC_IR的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_nec_ir_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为STRING
    unsigned char ret;
    /*
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_string_update(DPID_NEC_IR,Mec_Code, 4);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_ledred_handle
功能描述 : 针对DPID_LEDRED的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_ledred_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    ledred = mcu_get_dp_download_bool(value,length);
    if(ledred == 0) {
       LED_R=1; //开关关
    }else {
       LED_R=0; //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_LEDRED,ledred);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_ledblue_handle
功能描述 : 针对DPID_LEDBLUE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_ledblue_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    ledblue = mcu_get_dp_download_bool(value,length);
    if(ledblue == 0) {
      LED_G =1; //开关关
    }else {
      LED_G	=0;  //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_LEDBLUE,ledblue);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_wifi_control_handle
功能描述 : 针对DPID_WIFI_CONTROL的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_wifi_control_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char wifi_control;
    
    wifi_control = mcu_get_dp_download_bool(value,length);
    if(wifi_control == 0) {
       wifi_flag=0; //开关关
    }else {
      wifi_flag=1;   //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_WIFI_CONTROL,wifi_control);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_RELAY:
            //继电器控制处理函数
            ret = dp_download_relay_handle(value,length);
        break;
        case DPID_NEC_IT:
            //红外发射处理函数
            ret = dp_download_nec_it_handle(value,length);
        break;
        case DPID_NEC_IR:
            //红外接收处理函数
            ret = dp_download_nec_ir_handle(value,length);
        break;
        case DPID_LEDRED:
            //红灯处理函数
            ret = dp_download_ledred_handle(value,length);
        break;
        case DPID_LEDBLUE:
            //蓝灯处理函数
            ret = dp_download_ledblue_handle(value,length);
        break;
				case DPID_WIFI_CONTROL:
            //蓝灯处理函数
            ret = dp_download_wifi_control_handle(value,length);
        break;
