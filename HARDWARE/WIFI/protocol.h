...
/******************************************************************************
                       1:修改产品信息
******************************************************************************/
#define PRODUCT_KEY "a5ompnp5wsoj8amq"    //开发平台创建产品后生成的16位字符产品唯一标识

#define MCU_VER "1.0.0"         //用户的软件版本,用于MCU固件升级,MCU升级版本需修改
extern u16 temp_max;//温度初始值
extern u16 temp_min;//
extern u16 rh_max;//湿度初始值
extern u16 rh_min;
...
//继电器控制(可下发可上报)
//备注:用于查看继电器状态
#define DPID_RELAY 101
//红外发射(只下发)
//备注:发送红外协议内容
#define DPID_NEC_IT 102
//红外接收(可下发可上报)
//备注:接收并解码
//
#define DPID_NEC_IR 103
//当前光照值(只上报)
//备注:获取光照传感器的值
#define DPID_LIGHT_LESEN 104
//热释电传感器(只上报)
//备注:检测是否有人
#define DPID_INFRARED 105
//红灯(可下发可上报)
//备注:红灯
#define DPID_LEDRED 106
//蓝灯(可下发可上报)
//备注:绿灯控制
#define DPID_LEDBLUE 107

#define DPID_WIFI_CONTROL 108
extern u8 wifi_flag; 
/**
 * @brief  串口发送数据
